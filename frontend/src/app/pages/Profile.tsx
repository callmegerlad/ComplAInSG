...
export function ProfilePage() {
  ...
  const alertResponseCount = user?.alert_response_count ?? 0;
  const trustScore = Math.max(0, Math.min(100, user?.trust_score ?? 50));
  const badgeIds = user?.badges ?? [];
  const renderedBadges = badgeIds
    .map((badgeId) => BADGE_META[badgeId])
    ...
    <>
      ...
      <div className="mx-4 mb-6 rounded-xl bg-surface-1 shadow-card p-4 border border-border-subtle">
        ...
        <div className="h-1.5 w-full bg-accent-subtle rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-accent-primary to-success rounded-full"
      ...
      <div className="grid grid-cols-3 gap-3 mx-4 mb-8">
        {[
          ...
          { label: "Responses", value: alertResponseCount.toString() },
      ...
      <div className="mb-8">
        ...
        {renderedBadges.length > 0 ? (
          ...
          <div className="px-4 text-[12px] text-text-secondary">
            No badges yet. Submit reports to start earning them.
          </div>
        )}
      </div>
...
const BADGE_META: Record<string, BadgeMeta> = {
  first_report: { id: "first_report", label: "First Report", icon: "flag" },
  consistent_reporter: { id: "consistent_reporter", label: "Consistent", icon: "track_changes" },
  trusted: { id: "trusted", label: "Trusted", icon: "verified" },
  guardian: { id: "guardian", label: "Guardian", icon: "local_police" },
};
